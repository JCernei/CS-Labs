...
class Vigenere(Cypher):
    ...
    def encrypt(self, message):
        ...
        for letter_index, letter in enumerate(message):
            ...
            keyword_alpha_index = self.alpha.find(
                self.keyword[letter_index % len(self.keyword)])
            encrypted_letter = self.alpha[(
                letter_alpha_index + keyword_alpha_index) % 26]
